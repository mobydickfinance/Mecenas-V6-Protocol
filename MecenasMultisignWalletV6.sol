...
interface MecenasPool {
    ...
    function withdrawYield(uint _amount) external;
...
contract MecenasMultisignWalletV6 is ReentrancyGuard {
    ...
    constructor(address _owner, address _pooladdress, address _underlyingaddress) {
        ...
        require(_pooladdress != EMPTY_ADDRESS);
        require(_underlyingaddress != EMPTY_ADDRESS);
    ...
    function addPendingTransaction(uint _transtype, uint _amount, address _to) external nonReentrant {
        ...
        require(_amount > 0);
        require(_transtype == 1 || _transtype == 3 || _transtype == 4 || _transtype == 5 
    ...
    function _executeTransaction(uint _index) internal {
        ...
        if (transaction.transtype == 1) {
            walletpool.withdrawYield(transaction.amount);
